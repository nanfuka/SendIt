...
class AppTestCase(unittest.TestCase):
    ...
    # def test_index(self):
        ...
        # self.AssertionError("b'WELCOME TO SENDIT (CHALLENGE 3)'", str(response.data))


    def test_create_user(self):
        """tests the response data returned upon user signup
        """
        response = self.tefst_client.post('/api/v1/auth/signup', data=json.dumps(self.user), content_type ='application/json')
        self.assertEqual(201, response.status_code)
        self.assertIn ('b\'{"message":"user successfully signedup","user":{"email":"deb@gmail.com","username":"debbbs"}}\\n\'', str(response.data))
