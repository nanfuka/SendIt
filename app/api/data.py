...
class Userdata:
    ...
    def create_user(self):
        ...
        if not email or not re.match(r"([\w\.-]+)@([\w\.-]+)(\.[\w\.]+$)",
                                     email):
            return jsonify({
                "status": "Fail",
        ...
        return jsonify({"message": "user successfully signedup", 
                        "user": parcel}), 201
    ...
    def login_user(self):
        ...
        if any(char in specialCharacters for char in (user_data['username'])):
            return jsonify({'message': 'username cannot \
            have special characters'}), 400
        elif user_data['username'].isspace():
            return jsonify({"message": "Field cannot be blank"}), 400

        query = "SELECT * FROM users WHERE username = '{}'"
        self.database.cursor.execute(query.format(username))
        if self.find_user_by_name(user_data
                                    ['username']) and self.find_user_by_password(user_data['password']):
            token = jwt.encode({
                'username': user_data['username'],
                'exp':
                datetime.datetime.utcnow() + datetime.timedelta(minutes=30)
            }, 'userkey')
            return jsonify({'message': 'you have logged in as the \
             user',
    ...
    def create_parcel(self):
        ...
        query = """
            ...
            VALUES('{}','{}','{}','{}','{}','{}')
        """
        self.database.cursor.execute(query.format
                                     (user_data['user_id'],
                                      user_data['name_of_reciever'],
                                      user_data['source'],
                                      user_data['destination'],
                                      user_data['status'], 
                                      user_data['presentlocation']))
        return jsonify({"message": "order successfully \
        created", "parcel": user_data})
